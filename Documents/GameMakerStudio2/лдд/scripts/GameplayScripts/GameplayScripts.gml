...
/// @description Creates a health bar for an enemy
/// @param {num} _x_offset
/// @param {num} _y_offset
function create_health_bar(_x_offset, _y_offset)
{
	// Create an instance of obj_health_bar and access the internal values of this new instance using with
	with (instance_create_layer(_x_offset, _y_offset, "Instances", obj_health_bar)) 
	{
		// Set my enemy to the id of the instance that created it
		my_enemy = other.id;
		
		// Set the x_offset using the argument we passed in
		x_offset = _x_offset;
		
		// Set the y_offset using the argument we passed in
		y_offset = _y_offset;
	}
}
